...
module.exports = function paymentController(app) {
  ...
  /**
   * @api {getAccountDetails} /api/v1/payments/getAccountDetails
   * @apiName getAccountDetails
   * @apiGroup Result
   *
   */
  async function getAccountDetails(req, res) {
    const { account } = req.params; // Extract account from request params
    if (!account) {
      return res.status(400).json({ message: 'Account is required' });
    }
    const response = await app.payments.getAccountDetails(account); // Pass account to the service
    res.status(200).json(response);
  }
  ...
  return Object.freeze({
    ...
    getAccountDetails: asyncMiddleware(getAccountDetails),
  });
};
